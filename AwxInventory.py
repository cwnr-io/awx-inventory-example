...
def add_random_host(inventory, group):
    host = "host-" + str(random.randint(1000, 9999))
    inventory.add_host(host)
    inventory.add_host_vars(host, {"uuid": str(uuid.uuid4())})
    inventory.add_host_to_group(host, group)
    return None

if __name__ == "__main__":
    inventory = AwxInventory()
    for group in ["etcd", "kube-master", "kube-worker"]:
        inventory.add_group(group)
        inventory.add_group_vars(group, {"role": group})
    
    for group in ["etcd", "kube-master"]:
        for i in range(3):
            add_random_host(inventory, group)
    for group in ["kube-worker"]:
        for i in range(10):
            add_random_host(inventory, group)
